# -*- coding: utf-8 -*-
"""
Created on Wed July 3 11:08:55 2019
...
class AdaBoost:
    def __init__(self,X,y,Weaker=DecisionStump): # 使用的弱分类器为 DecisionStump
        self.X=np.array(X)
    ...
    def train(self,M=5):  # M 为弱分类器的最大数量，可以在main函数中修改，M 默认为5
        self.G={}         # 表示弱分类器的字典
    ...
    def pred(self,test_X):  #测试最终的分类器
        ...
        sums=np.zeros(test_X.shape[1]) # test_X.shape[1] 表示数据集test_X的大小
